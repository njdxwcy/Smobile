...
def mobile_evaluate(opt,
    ...
    dataloader, testloader = get_loaders(opt.data, batch_size, batch_size, imgsz,
                                           nw)
    ...
    with torch.no_grad():
        for batch_i, (imgs, targets) in enumerate(tqdm(dataloader, total=nb)):
            ...
            pred=F.log_softmax(output_ann_true.view(output_ann_true.size(0),-1), dim=1)
            pred_max=pred.max()
            ...
            cnt+=batch_size
            print('accuracy:',100. * acc_top1 / cnt,100. * acc_top5 / cnt,pred_max)
