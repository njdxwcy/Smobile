...
def create_modules(module_defs, img_size, cfg):
    ...
    for i, mdef in enumerate(module_defs):
        ...
        if mdef['type'] == 'convolutional':
            ...
            k = mdef['size']  # kernel size
            stride = mdef['stride']
            avgpool = nn.AvgPool2d(kernel_size=k, stride=stride)
            modules = avgpool
            ...
            size=mdef['size']
            filters = mdef['filters']
            modules.add_module('Linear', nn.Linear(size, filters))
...
def load_darknet_weights(self, weights, cutoff=-1):
    ...
    for i, (mdef, module) in enumerate(zip(self.module_defs[:cutoff], self.module_list[:cutoff])):
        if mdef['type'] == 'convolutional':
            ...
            linear=module[0]
            nb = linear.bias.numel()
            linear_b = torch.from_numpy(weights[ptr:ptr + nb]).view_as(linear.bias)
            linear.bias.data.copy_(linear_b)
            ptr += nb
            nw = linear.weight.numel()  # number of weights
            linear.weight.data.copy_(torch.from_numpy(weights[ptr:ptr + nw]).view_as(linear.weight))
            ptr += nw
    ...
    with open(path, 'wb') as f:
        ...
        for i, (mdef, module) in enumerate(zip(self.module_defs[:cutoff], self.module_list[:cutoff])):
            if mdef['type'] == 'convolutional':
                ...
                linear_layer.bias.data.cpu().numpy().tofile(f)   
                linear_layer.weight.data.cpu().numpy().tofile(f)   
