...
def train(model, loader, epoch, optimizer, criterion, device, dtype, batch_size, log_interval, scheduler):
    ...
    for batch_idx, (data, target) in enumerate(tqdm(loader)):
        ...
        optimizer.zero_grad()
        output = model(data)
        output_new=output.view(output.size(0),-1)
        out=F.log_softmax(output_new, dim=1)
...
def save_checkpoint(state, is_best, filepath='./', filename='checkpoint.pt'):
    save_path = os.path.join(filepath, filename)
    best_path = os.path.join(filepath, 'model_best.pt')
